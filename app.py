...
def cost():
                ...
                if serial_number not in processed_serial_numbers:
                    ...
                    if row[7].strip('{}') != '{nan}':
                        ...
                        try:
                            ...
                            ordered_values = [float(process_params[key]) if key in process_params else 0 for key in
                                              process_parameter_keys]
    ...
    a = get_coal_percentages_csv()
    D = np.loadtxt(io.StringIO(a), delimiter=',')
    an = get_Individual_coal_properties_csv()
    P = np.loadtxt(io.StringIO(an), delimiter=',')
    # coke_properties
    ani = get_coke_properties_csv()
    Coke_properties = np.loadtxt(io.StringIO(ani), delimiter=',')
    anir = get_coal_properties_csv()
    data = pd.read_csv(io.StringIO(anir), dtype=str, header=None, on_bad_lines='skip')
    ...
    aniru = get_blended_coal_properties_csv()
    Blended_coal_parameters = np.loadtxt(io.StringIO(aniru), delimiter=',')
    ...
    for i, blend in enumerate(sorted_blends):
        ...
        for j, coal_type in enumerate(coal_types):
            if j < len(blend):
                ...
                coal_type_cost = float(data.loc[data[0] == coal_type, data.columns[-1]].values[0])
                coal_type_costs.append(coal_type_cost)
        coal_costs.append(coal_type_costs)

    total_costs = [sum(float(blend[i]) * coal_costs[j][i] / 100 for i in range(min(len(blend), len(coal_costs[j])))) for
                   j, blend in enumerate(sorted_blends)]
